...
import { IconCube, IconClose, IconCheck } from "./Icons";

const GUIDE_SEEN_KEY = "wp-builder-guide-seen";
...
export function BuilderWrapper({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (state.isActive && !sessionStorage.getItem(GUIDE_SEEN_KEY)) {
      setShowGuide(true);
    }
  }, [state.isActive]);
  ...
  // Keyboard shortcut: Cmd/Ctrl+Shift+B to toggle
  useEffect(() => {
    ...
    <>
      ...
      {isAdmin && (
        <button
          ...
          className={`fixed bottom-6 left-6 z-[90] w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 ${
            state.isActive
              ? "bg-espresso text-white scale-110"
              : "bg-gold text-espresso hover:scale-110"
          }`}
          title="Toggle Builder (Cmd+Shift+B)"
        >
          {state.isActive ? (
        ...
        >
          ...
          >
            <div className="text-center mb-6">
              ...
              <h2 className="font-serif text-2xl text-heading mb-2">Builder Access</h2>
              <p className="text-sm text-text-gray">Enter the admin password to access the drag-and-drop page builder.</p>
      ...
      {/* Hidden trigger: bottom-right small button for non-admins (invisible) */}
      {!isAdmin && (
        <button
          onClick={() => setShowLogin(true)}
          className="fixed bottom-2 right-2 z-[80] w-6 h-6 rounded-full bg-espresso/5 hover:bg-espresso/15 text-text-gray/30 hover:text-text-gray/70 flex items-center justify-center transition-all"
      ...
      {/* Admin badge when logged in but builder closed */}
